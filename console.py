...
Assumptions: You can assume arguments are always in the right order
             ...
             The error management starts from the first
             argument to the last one
...
class HBNBCommand(cmd.Cmd):
    ...
    def do_destroy(self, dest_arg):
        ...
        if len(input_cmd) == 0:
            ...
            objects = storage.all()
            instance_key = "{}.{}".format(input_cmd[0], input_cmd[1])
    ...
    def do_all(self, arg):
        ...
        if len(input_cmd) == 0:
            for key, value in objects.items():
                print(str(value))  # Output must be in string representation
    ...
    def default(self, arg):
        """Implement default Behavior for cmd module."""
        input_cmd = arg.split('.')
        # input_cmd[0] = User
        ...
        # 2. Implement <class name>.count()
            ...
            objects = storage.all()
            cnt = sum(1 for key in objects.keys() if input_classname in key)
            print(cnt)
